...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .atlas-frame-inner { fill: rgba(255,255,255,0.4); stroke: rgba(23,37,34,0.12); stroke-width: 1; }
    .map-title, .north { fill: var(--ink); font: 700 17px Inter, "Segoe UI", sans-serif; }
    .route-shadow {
      fill: none;
    ...
    .city-label {
      ...
      stroke-width: 6;
      fill: var(--brand-dark);
      font: 750 15px Inter, "Segoe UI", sans-serif;
    }
    ...
    .marker-num { fill: #fff; font: 750 14px Inter, "Segoe UI", sans-serif; }
    .poi-label {
      pointer-events: none;
      paint-order: stroke;
    ...
    .scale-bar line { stroke: #41514d; stroke-width: 2; }
    .scale-bar text { fill: #41514d; font: 650 12px Inter, "Segoe UI", sans-serif; }
    .summary-grid {
      display: grid;
  ...
  <script>
    ...
    function projectStops(items, width = MAP_W, height = MAP_H, padding = 72) {
      ...
      values.forEach(({ stop, x, y }) => {
        const originX = Math.round((left + (x - minX) * fit) / 8) * 8;
        const originY = Math.round((top + (maxY - y) * fit) / 8) * 8;
        let px = Math.min(Math.max(originX, 44), width - 44);
        let py = Math.min(Math.max(originY, 44), height - 44);
        for (const [dx, dy] of offsets) {
          const candidateX = Math.min(Math.max(originX + dx, 44), width - 44);
          const candidateY = Math.min(Math.max(originY + dy, 44), height - 44);
          if (placed.every(point => Math.hypot(candidateX - point[0], candidateY - point[1]) >= 34)) {
            px = candidateX;
            py = candidateY;
            break;
          }
