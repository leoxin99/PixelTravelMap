...
1. **加载既有计划**：点击“加载示例行程”，页面直接进入 7 日时间线。
...
设计并实现 PixelTravelMap 离线旅行行程助手，将 Word/文字行程结构化为统一数据模型；针对晚点、闭馆、天气和体力变化提供可解释的局部重规划，保护固定预约并同步更新时间线、城市关系地图、同行简报及离线导出。建立公开示例、本地匿名事件记录与静态产物质量检查，并明确区分 Prototype 能力和待验证产品指标。
